...
def parse_mdvr_filename(filename: str) -> dict:
    ...
    if edge_match:
        return {"subject_id": edge_match.group(1).upper(), "class": edge_match.group(2).lower()}

    raise ValueError(f"Cannot parse filename: {filename}")


def discover_recordings(task: str, base_dir: Optional[Path] = None) -> list[RecordingInfo]:
    """
...
def build_subject_registry(recordings: list[RecordingInfo]) -> dict[str, dict]:
    """
...
def load_dataset_manifest(task: str, base_dir: Optional[Path] = None) -> pd.DataFrame:
    """
