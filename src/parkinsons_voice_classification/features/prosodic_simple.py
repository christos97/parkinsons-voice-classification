...
def extract_prosodic_features(audio_path: str) -> dict:
    ...
    try:
        ...
        pitch = call(sound, "To Pitch (ac)", 0.0, F0_MIN_HZ, 15, "no", 0.03, 0.45, 0.01, 0.35, 0.14, F0_MAX_HZ)
    ...
    try:
        ...
        features.update({k: np.nan for k in ['f1_mean', 'f2_mean', 'f3_mean', 'f1_std', 'f2_std', 'f3_std']})
