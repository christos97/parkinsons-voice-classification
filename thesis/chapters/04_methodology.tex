...
\subsubsection{Fundamental Frequency (\texorpdfstring{$F_0$}{F0})}
...
Five classical ML models were selected to provide diverse inductive biases while maintaining interpretability:
...
\begin{itemize}
    ...
    \item \textbf{Gradient Boosting (GB):} Sequential ensemble that iteratively corrects prediction errors via gradient descent in function space. Tests whether adaptive boosting of weak learners improves discrimination.
    \item \textbf{XGBoost (XGB):} Regularised gradient boosting framework with L1/L2 penalties and efficient tree construction~\cite{chen2016xgboost}. Tests whether explicit regularisation provides additional robustness on small datasets.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Diversity} --- Represent linear, kernel-based, bagging, and boosting approaches
\end{itemize}
...
Random Forest & Bagging & n\_estimators$=100$, max\_depth$=10$, min\_samples\_split$=2$, random\_state$=42$ \\
Gradient Boosting & Boosting & n\_estimators$=100$, learning\_rate$=0.1$, max\_depth$=3$ (default), random\_state$=42$ \\
XGBoost & Boosting & n\_estimators$=100$, learning\_rate$=0.1$, eval\_metric=`logloss', random\_state$=42$ \\
\bottomrule
\end{tabular}
...
Gradient Boosting constructs an additive model in a stage-wise fashion, where each new tree corrects the residual errors of the ensemble~\cite{friedman2001greedy}:
...
where $F_m$ is the ensemble after $m$ stages, $h_m$ is the weak learner (decision tree) fitted to the negative gradient of the loss function, and $\eta = 0.1$ is the learning rate. The learning rate shrinks the contribution of each tree, providing a regularisation effect.

\subsubsection{XGBoost}

XGBoost~\cite{chen2016xgboost} extends gradient boosting with explicit L1 and L2 regularisation on leaf weights:
...
Here $T$ is the number of leaves, $\mathbf{w}$ is the vector of leaf weights, and $\gamma$, $\lambda$ are regularisation parameters. This regularisation helps prevent overfitting, which is particularly relevant for small-sample datasets.
...
Three of the five models (Logistic Regression, SVM, Random Forest) support the \texttt{class\_weight} parameter natively via scikit-learn. The two boosting classifiers (Gradient Boosting and XGBoost) do not support \texttt{class\_weight="balanced"} natively; however, since the mild class imbalance (1.3:1) did not require reweighting in practice (see Chapter~\ref{ch:results}), this difference has no impact on the experimental design.
...
In this thesis, \textbf{$K$-fold cross-validation serves as the primary validation strategy}: the model is trained on $K-1$ folds and evaluated on the remaining fold, which contains only samples that the model has never seen during training or feature standardisation~\citep{bishop2006pattern}. This procedure is repeated $K=5$ times, producing five independent estimates of generalisation performance. A separate held-out test set was not used because, with only $n=37$ subjects (Dataset A), reserving even 20\% for testing would yield approximately seven test subjects per fold under subject-level splitting~--- too few for stable metric computation. Cross-validation maximises data utility while still guaranteeing that each evaluation uses strictly unseen data.

Since all hyperparameters were fixed \emph{a priori} (Table~\ref{tab:model-specs}), no inner hyperparameter-selection loop is required: the 5-fold CV here serves purely as an \textbf{evaluation mechanism}, not a tuning mechanism. This eliminates any risk of optimising hyperparameters towards the held-out data.
...
\begin{itemize}
    ...
    \item \textbf{Model:} Logistic Regression vs SVM vs Random Forest vs Gradient Boosting vs XGBoost
\end{itemize}

This yields 20 conditions per dataset/task combination:
...
\midrule
\textbf{Unweighted} & 5 models & 5 models \\
\textbf{Weighted} & 5 models & 5 models \\
\bottomrule
\end{tabular}
\caption{2$\times$2$\times$5 factorial design: 20 conditions total}
\label{tab:factorial-design}
\end{table}

For Dataset A (MDVR-KCL), experiments were conducted separately for each speech task (ReadText and SpontaneousDialogue), yielding $20 \times 2 = 40$ experimental conditions. Dataset B (PD Speech Features) yielded 20 conditions.
...
\begin{itemize}
    ...
    \item Random Forest, Gradient Boosting, XGBoost: \texttt{random\_state=42}
...
\begin{enumerate}
    ...
    \item Five classical ML models with fixed hyperparameters
