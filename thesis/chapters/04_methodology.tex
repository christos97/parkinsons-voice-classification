...
All parameters (random seed 42; 22\,050\,Hz sample rate; $F_0$ range 75--500\,Hz; 13 MFCCs with 2048-point FFT, 512-sample hop, 128 mel bands) were fixed in a central configuration module before experimentation, with results saved to CSV for all downstream experiments.

\section{Machine Learning Models}

\subsection{Model Specifications}

Five classical ML models were selected for diverse inductive biases: Logistic Regression (linear baseline with L2 regularisation), SVM with RBF kernel (nonlinear kernel method), Random Forest (ensemble averaging with built-in feature importance), Gradient Boosting~\cite{friedman2001greedy} (sequential residual correction), and XGBoost~\cite{chen2016xgboost} (regularised boosting). All hyperparameters were fixed \emph{a priori} without dataset-specific tuning:
...
Features are standardised to zero mean and unit variance (\texttt{StandardScaler}) fitted exclusively on the training fold within each CV split, preventing information leakage from the test data.
