...
A research demonstration interface was implemented to visualize per-recording model outputs; however, these outputs were not used in the quantitative evaluation reported in this thesis (Appendix~\ref{ch:demo-app}).
...
% TODO: Update hierarchy after experiment re-run with actual boosting results
...
\subsection{Validation Strategy: Cross-Validation over Train/Test Split}

A deliberate methodological choice was to use five-fold cross-validation as the sole evaluation mechanism rather than a dedicated held-out test set. For datasets of this scale ($n \leq 37$ recording subjects with subject-level splits), a single train/test partition is problematic:

\begin{itemize}
    \item A 20\% hold-out retains only $\sim$7 subjects for final evaluation~--- a sample too small for stable metric computation (one misclassification shifts accuracy by $\sim$14\%).
    \item A single split produces a point estimate with no associated variance, obscuring whether the result reflects genuine generalisation or the specific subjects in the test set.
    \item Cross-validation exposes the high fold-to-fold variability inherent to small medical cohorts, providing more honest uncertainty characterisation (observed std $= 0.15$--$0.30$ in this study).
\end{itemize}

Crucially, because all hyperparameters were fixed before any data were examined, no separate validation set was needed for model selection. The CV folds serve one purpose: estimating generalisation performance on unseen subjects. Feature standardisation (\texttt{StandardScaler}) is performed strictly within each fold~--- fitted on the training partition and applied to the test partition~--- so that no information about the test fold influences any training or preprocessing step.
...
\begin{itemize}
    ...
    \item \textbf{Gradient Boosting:} \textit{Pending experiment re-run}
    \item \textbf{XGBoost:} \textit{Pending experiment re-run}
