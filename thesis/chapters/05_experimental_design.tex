...
\begin{itemize}
    \item \textbf{No held-out test set:} Performance estimated via 5-fold cross-validation only. A separate test set was deliberately omitted: with $n=37$ subjects (Dataset A), a 20\% hold-out at the subject level would yield only $\sim$7 test subjects~--- producing single-split estimates highly sensitive to which subjects happen to fall in that set. A one-subject swing in classification shifts accuracy by $\sim$14\%. Cross-validation returns five independent fold scores, providing variance-characterised estimates while maximising training data per fold.
...
\begin{enumerate}
    \item A comprehensive $2 \times 2 \times 5$ factorial design (20 conditions per dataset)
...
The next chapter presents the results from these 300 total experimental runs across all conditions, datasets, and models.